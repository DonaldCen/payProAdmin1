<template>
  <a-card :bordered="false" class="card-area">
        ...
        <div :class="advanced ? null: 'fold'">
            ...
            <a-col :md="12" :sm="24" >
              <a-form-item
                ...
                :labelCol="{span: 4}"
                :wrapperCol="{span: 18, offset: 2}">
                <a-input v-model="queryParams.channelCode"/>
              </a-form-item>
            </a-col>
    ...
    <div>
      <div class="operator">
        <a-button type="primary" ghost @click="add" v-hasPermission="'channel:add'">新增</a-button>
        <a-button @click="batchDelete" v-hasPermission="'channel:delete'">删除</a-button>
      </div>
      <!-- 表格区域 -->
      <a-table ref="TableInfo"
        ...
        <template slot="operation" slot-scope="text, record">
          <a-icon v-hasPermission="'channel:update'" type="setting" theme="twoTone" twoToneColor="#4a9ff5" @click="edit(record)" title="修改用户"></a-icon>
          &nbsp;
          <a-icon v-hasPermission="'channel:view'" type="eye" theme="twoTone" twoToneColor="#42b983" @click="view(record)" title="查看"></a-icon>
          <a-badge v-hasNoPermission="'channel:update','channel:view'" status="warning" text="无权限"></a-badge>
        </template>
      </a-table>
    </div>
    <!-- 信息查看 -->
...
export default {
  name: 'User',
  components: {ChannelInfo, ChannelAdd, ChannelEdit, DeptInputTree, RangeDate},
  data () {
    return {
      advanced: false,
      channelInfo: {
  ...
  computed: {
    columns () {
      ...
      return [{
        ...
        onFilter: (value, record) => record.settlementType.includes(value)
      }, {
        title: '支付限额',
        dataIndex: 'payLimit'
      }, {
        ...
        customRender: (text, row, index) => {
          switch (text) {
            ...
            case '0':
              return <a-tag color="cyan">可用</a-tag>
            default:
              return text
          }
        },
  ...
  methods: {
    ...
    handleUserInfoClose () {
      this.channelInfo.visiable = false
    },
